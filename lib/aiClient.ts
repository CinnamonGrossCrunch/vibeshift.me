...
export async function runAI(opts: RunAIOptions): Promise<RunAIResult> {
  ...
  for (const model of chain) {
    ...
    let attemptedReasoning = supportsReasoning;
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        if ((c as any).responses) {
          const body: any = { model, input: prompt, temperature: temp };
          if (supportsReasoning) {
            body.reasoning = { effort: reasoning };
            body.text = { verbosity };
          }
          const r = await (c as any).responses.create(body);
          const text = (r as any).output_text || (r as any).content?.map((p: any) => p.text?.value).join('\n') || '';
        ...
        if (e?.status === 400 && /(model|Unsupported parameter)/i.test(msg)) {
  ...
  throw new Error(`All models failed (${tried.join(' -> ')}). Last error: ${lastErr?.message || lastErr}`);
}
